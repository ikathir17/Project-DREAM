...
const ManualVerification = () => {
  ...
  const handleVerification = async (complaintId, verified, isReVerification = false) => {
    ...
    try {
      ...
      const response = isReVerification 
  ...
  const openVerificationModal = (complaint, verified, isReVerification = false) => {
  ...
  const currentComplaints = activeTab === 'pending' ? pendingComplaints : rejectedComplaints;
                ...
                <div className="modal-content">
                  ...
                  <div className="reason-input">
                    <label htmlFor="reason">
                      Reason for {selectedComplaint.isReVerification ? 're-' : ''}{selectedComplaint.verificationDecision ? 'verification' : 'rejection'}:
                    </label>
                    <textarea
                      id="reason"
                      value={verificationReason}
                      onChange={(e) => setVerificationReason(e.target.value)}
                      placeholder={`Please provide a reason for ${selectedComplaint.isReVerification ? 're-' : ''}${selectedComplaint.verificationDecision ? 'verifying' : 'rejecting'} this complaint...`}
                      rows={4}
                      required
                    />
                  </div>
